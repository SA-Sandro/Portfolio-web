<template>
  ...
  >
    <input
      ...
      type="text"
      readonly
      class="cursor-pointer w-60 border-2 text-sm rounded-lg block ps-8 p-2 border-black bg-white text-black"
      value="sandrosuarezaranda@gmail.com"
    />
    <button
...
const copyToClipboard = async (): Promise<void> => {
  const inputValue = (
    document.querySelector("#mail") as HTMLInputElement
  ).value;
